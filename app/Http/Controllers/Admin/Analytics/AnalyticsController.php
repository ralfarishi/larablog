  ...
  public function index(): View
  {
    ...
    $analytics = Cache::remember('admin_analytics', 900, function () {
      ...
      $publishedLastWeek = Post::where('status', 'published')
        ->whereBetween('created_at', [$lastWeek, $lastWeekEnd])->count();
      ...
      $weeklyPosts = $posts->groupBy(fn ($post) => $post->created_at->format('oW'))
      ...
      $totalEngagement = $postsWithCounts->sum(fn ($p) => ($p->bookmarks_count * 2) + $p->comments_count);
      $avgEngagement   = $postsWithCounts->count() > 0
      ...
      $recentComments = Comment::with(['user:id,name', 'post:id,title'])
        ...
        ->map(fn ($item) => [
          ...
          'content' => 'Commented: "' . \Illuminate\Support\Str::limit($item->content, 40) . '"',
