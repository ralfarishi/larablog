<section class="bg-background min-h-screen py-12 md:py-20">
  <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
    {{-- Header --}}
    <div
      class="border-border mb-10 flex flex-col justify-between gap-6 border-b pb-6 sm:flex-row sm:items-center"
    >
      <div>
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-4">
      ...
      <div class="lg:col-span-1">
        <div class="bg-card ring-border sticky top-24 rounded-[2.5rem] p-6 shadow-sm ring-1">
          <nav class="space-y-2">
            <button
              type="button"
            ...
            <div class="bg-card ring-border rounded-[2.5rem] p-8 shadow-sm ring-1 sm:p-10">
              ...
              @if ($bookmarks->isEmpty())
                <div class="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p class="text-muted-foreground mt-1 max-w-xs text-sm">Articles you bookmark will appear here for easy access.</p>
                </div>
              @else
                  ...
                  @foreach ($bookmarks as $bookmark)
                    ...
                    @if ($post)
                      <article
                        ...
                        class="bg-muted/30 ring-border group relative flex flex-col overflow-hidden rounded-3xl ring-1 transition-all duration-300 hover:-translate-y-1 hover:shadow-md"
                      >
                        <a
                          href="{{ route('post', $post->slug) }}"
                          class="bg-muted relative block h-40 overflow-hidden"
                        >
                          <img
                            src="{{ $post->image_url }}"
                            alt="{{ $post->title }}"
                        ...
                        <div class="flex flex-1 flex-col p-5">
                          @if ($post->category)
                            <span
                              class="bg-primary/10 text-primary mb-2 inline-flex items-center self-start rounded-full px-2.5 py-0.5 text-[10px] font-black tracking-widest uppercase"
                            >
                              {{ $post->category->name }}
                            </span>
                          @endif
                          <h5
                            class="text-foreground group-hover:text-primary mb-2 line-clamp-2 text-base font-bold transition-colors"
                          >
                          ...
                          >
                            <span class="text-muted-foreground text-xs font-semibold"
                              >By {{ $post->user->name ?? 'Author' }}</span
                            >
                            <button
                              type="button"
                              wire:click="removeBookmark({{ $bookmark->id }})"
                              class="rounded-lg p-1.5 text-red-500 transition-colors hover:bg-red-500/10 hover:text-red-700"
            ...
            <div class="bg-card ring-border rounded-[2.5rem] p-8 shadow-sm ring-1 sm:p-10">
              ...
              @if ($comments->isEmpty())
                <div class="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p class="text-muted-foreground mt-1 max-w-xs text-sm">Your comments on articles will be logged here.</p>
                </div>
              @else
                <div class="space-y-6">
                  @foreach ($comments as $comment)
                    <div
                      wire:key="dashboard-comment-{{ $comment->id }}"
                      class="bg-muted/30 border-border hover:bg-muted/50 rounded-2xl border p-5 transition-all"
                    >
                      <div class="mb-3 flex items-start justify-between gap-4">
                        <div class="min-w-0">
                          ...
                          <time
                            class="text-muted-foreground mt-0.5 block text-[10px] font-bold uppercase"
                          >
                        ...
                        <button
                          ...
                          class="shrink-0 rounded-lg p-2 text-red-500 transition-colors hover:bg-red-500/10 hover:text-red-700"
                          title="Delete Comment"
                        >
                          <i class="ph ph-trash text-base"></i>
                        </button>
                      </div>
                      <p class="text-foreground bg-background/50 border-border/40 rounded-xl border p-4 text-sm leading-relaxed font-medium">
                        {{ $comment->content }}
                      </p>
                    </div>
                  @endforeach
              ...
              <form wire:submit.prevent="updateProfile" class="space-y-6">
                ...
                <div class="flex flex-col items-center gap-4 sm:flex-row">
                  <div class="group relative">
                    <div class="ring-border h-24 w-24 overflow-hidden rounded-full ring-4">
                      @if ($avatar)
                        <img
                  ...
                  <div class="text-center sm:text-left">
                    <p class="text-foreground text-sm font-bold">Profile Picture</p>
                    <p class="text-muted-foreground mt-1 text-xs">Upload a PNG, JPG, JPEG, or WEBP image. Max size 1MB.</p>
                    @error ('avatar')
                ...
                <div class="bg-muted/20 border-border/60 space-y-4 rounded-2xl border p-6">
                  <h5 class="text-foreground text-sm font-black tracking-wider uppercase">
                    Change Password
                  </h5>
                ...
                <div class="flex justify-end pt-4">
                  <button
                    ...
                    class="bg-primary text-primary-foreground inline-flex items-center gap-2 rounded-xl px-6 py-3.5 text-xs font-black tracking-widest uppercase transition-all duration-300 hover:opacity-90 active:scale-95 disabled:opacity-75"
                  >
